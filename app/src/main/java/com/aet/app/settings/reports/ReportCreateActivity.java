...
public class ReportCreateActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_report_create_activity);
    ...
    private void InitView() {
        ...
        txtMulai.setOnClickListener(v -> {
            ...
            // date picker dialog
            picker = new DatePickerDialog(ReportCreateActivity.this,
    ...
    class GetReport extends AsyncTask<String, String, String> {
        ...
        private String URL_REPORT = Constants.BASE_URL + "api/report/create";
        private JSONParser jsonParser = new JSONParser();

        @Override
        protected void onPreExecute() {
            super.onPreExecute();
            progressDialog = new ProgressDialog(ReportCreateActivity.this);
        ...
        protected void onPostExecute(String s) {
            ...
            Toast.makeText(ReportCreateActivity.this, "PDF sudah dibuat di folder Downloads", Toast.LENGTH_LONG).show();
        }
        ...
        protected String doInBackground(String... strings) {
            ...
            if (spinnerFilter.getSelectedItemPosition() == 0) {
                ...
                Log.d("report", json.toString());
                try {
                    AllWithDetails(json.toString(), "create_all");
                } catch (IOException e) {
                    e.printStackTrace();
                }
                ...
                Log.d("report", json.toString());
                try {
                    All(json.toString(), "create_bydate");
                } catch (IOException e) {
                    e.printStackTrace();
                }
            } else {
                ...
                Log.d("report", json.toString());
                try {
                    AllWithDetails(json.toString(), "create_bydate");
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
    ...
    private void All(String json, String filename) throws IOException {
        ...
        document.add(new Paragraph("REKAP LATIHAN CREATE").setTextAlignment(TextAlignment.CENTER));
    ...
    private void AllWithDetails(String json, String filename) throws IOException {
        ...
        List<List<String>> create = new ArrayList<>();
        ...
        try {
            ...
            if (status == 200) {
                ...
                for (int i = 0; i < arr.length(); i++) {
                    ...
                    tanggal.add(obData.optString("updated_at"));
                    nilai.add(obData.optString("nilai"));
                    komentar.add(obData.optString("komentar"));
                    create.add(new ArrayList<>());
                    create.get(i).add(obData.optString("thesis"));
                    create.get(i).add(obData.optString("argument_1"));
                    create.get(i).add(obData.optString("argument_2"));
                    create.get(i).add(obData.optString("argument_3"));
                    create.get(i).add(obData.optString("reiteration"));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
        ...
        PdfFont font = PdfFontFactory.createFont(FontConstants.HELVETICA);
        document.add(new Paragraph("REKAP LATIHAN CREATE").setTextAlignment(TextAlignment.CENTER));
        float[] columnWidth = {120, 200, 80, 50, 200, 300};
        Table table = new Table(columnWidth);
        ...
        table.addCell(Constants.createCell("Detail", 1, 1, 15));
        for (int i = 0; i < nis.size(); i++) {
            String[] tgl = tanggal.get(i).split("T", 2);
            table.setTextAlignment(TextAlignment.LEFT);
            table.addCell(Constants.createCell(nis.get(i), 1, create.get(i).size() + 1, 15));
            table.addCell(Constants.createCell(nama.get(i), 1, create.get(i).size() + 1, 15));
            table.addCell(Constants.createCell(tgl[0], 1, create.get(i).size() + 1, 15));
            table.addCell(Constants.createCell(nilai.get(i), 1, create.get(i).size() + 1, 15));
            table.addCell(Constants.createCell(komentar.get(i), 1, create.get(i).size() + 1, 15));
            table.addCell(Constants.createCell("Create", 1, 1, 15));
            for (int j = 0; j < create.get(i).size(); j++) {
                table.addCell(Constants.createCell(create.get(i).get(j), 1, 1, 15));
            }
        }
